  ...
  */
  ...
  theme.loadCarousels = function (container) {
    return $('.carousel', container).each(function () {
      ...
      // create options
      if ($("collection-list-mobile")[0]){
        if ($(this).hasClass('mobile-product-list' && 'product-list--per-row-6')){
           var slideMain = 6
        }else if ($(this).hasClass('mobile-product-list' && 'product-list--per-row-5')) {
           var slideMain = 5
        }else if ($(this).hasClass('mobile-product-list' && 'product-list--per-row-4')) {
           var slideMain = 4
        }else if ($(this).hasClass('mobile-product-list' && 'product-list--per-row-3')) { 
           var slideMain = 3
        }else if ($(this).hasClass('mobile-product-list' && 'product-list--per-row-2')) { 
           var slideMain = 2 
      	}else {
           var slideMain = 2
        }
      ...
      if (fixedMode) {
        ...
        if ($(this).hasClass('mobile-product-list')){
        carouselOptions = {
          margin: 0,
          loop: true,
          autoWidth: false,
  ...
  theme.lazyBGSetWidths = [180, 360, 540, 720, 900, 1080, 1296, 1512, 1728];
      ...
      $form.on('submit', function (evt) {
        ...
        if (theme.settings.cart_type === 'drawer' && $formContainer.is('[data-ajax-add-to-cart="true"]')) {
          ...
          mmajaxcart.Ajaxupsell();
            ...
            window._izq.push(["trigger","product_addedtocart",{ }]);
            window._izq.push(["trigger","added_to_cart",{ "product_name": addedDataJSON.product_title, "product_price": addedDataJSON.final_price, "product_image":addedDataJSON.image, "cart_url": "https://www.kushals.com/cart" }])
  ...
  theme.LoadFilterer = function (section) {
    ...
    $(document).on('click' + this.namespace, function () {
      // var element = document.querySelectorAll(".utility-bar__item  .ct_fil_shade");  
      // for (var i = 0; i < element.length; i++) {
      //      element[i].classList.remove('ct_fill_active');
      //   }
        
        //element.classList.remove("ct_fill_active");
  ...
  theme.Navigation = {
    init: function init(options) {
      ...
      // create it
      var $mobileDrawer = $('<div class="mobile-navigation-drawer ct_mobile-navigation-drawer">');
  ...
  theme.initProductGallery = function (container) {
    ...
    if ($thumbnails.length) {
      var carouselVisibleItemCount = 5,
      isCarousel = false;
  ...
  theme.OptionManager = new function () {
    ...
    _.updatePrice = function (variant, $container) {
      ...
      var $price = $('<div class="price h5-style">').appendTo($newPriceArea);
      ...
      if (pageWidth > 767) {
        ...
        if ($block.toggleClass('expanded').hasClass('expanded')) {
          ...
          theme.currentQuickbuyRequest = $.get(url, function (response) {
            ...
            $quickbuyCont.find(".Color .is-unavailable,.Colour .is-unavailable").each(function() {
              let unavailableEmnt  = $(this).next('label:first');
              $('.option-selector__btns.Color,.option-selector__btns.Colour').append($(this));
              $('.option-selector__btns.Color,.option-selector__btns.Colour').append(unavailableEmnt);
            });
      ...
      updateCart: function updateCart(params, successCallback) {
        ...
        this.cartXhr = $.ajax({
          ...
          success: function success(data) {
            document.documentElement.dispatchEvent(
            new CustomEvent('theme:cartchanged', { bubbles: true, cancelable: false, detail: data }));
...
// main banner slider section code  slideshow
...
//     var $status = $('.ct_pagingInfo');
// var $slickElement = $('.CT_slideshow');
...
$( document ).ready(function() {
  $(".product-detail .Color .is-unavailable,.product-detail .Colour .is-unavailable").each(function() {
    let unavailableEmnt  = $(this).next('label:first');
    $('.option-selector__btns.Color,.option-selector__btns.Colour').append($(this)); 
    $('.option-selector__btns.Color,.option-selector__btns.Colour').append(unavailableEmnt);
  });
